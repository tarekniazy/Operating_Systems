...
int main(int argc, char *argv[])
{
   ...
   quantum=atoi(argv[4]);
    ...
    if(schedPID==0)
    {
        ...
        sprintf(policyNumbers,"%d",policyNumber);

    	char*const scheduler[]={"./scheduler.o",algorithm,quant,numberofprocesses,policyNumbers,NULL};
	...
	else
	{
		...
		sprintf(numberofprocesses,"%d",count);
        sprintf(policyNumbers,"%d",policyNumber);
    	char*const scheduler[]={"./scheduler.o",algorithm,numberofprocesses,policyNumbers,NULL};
    	execvp(scheduler[0],scheduler);
	}
